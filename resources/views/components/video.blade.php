...
<div x-data="{ playing: false, muted: false }" class="relative h-full w-full m-auto" @click.outside="$refs.player.pause()"
    x-intersect:leave="$ref.player.pause()">
    ...
    @if ($controls == true)
        ...
        <div class="absolute z-[100] bottom-2 right-2 m-4 bg-gray-900 text-white rounded-lg p-1 cursor-pointer">
            ...
            <svg x-cloak x-show="muted" @click="$refs.player.muted.false;muted=false" xmlns="http://www.w3.org/2000/svg"
                width="16" height="16" fill="currentColor" class="bi bi-volume-off-fill w-4 h-4"
                viewBox="0 0 16 16">
                <path
