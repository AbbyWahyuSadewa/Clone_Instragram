<div class="lg:px-8 my-auto h-full flex flex-col overflow-hidden">
    <ul class="h-[calc(100vh_-_1rem)] flex flex-col my-auto gap-y-6 snap-y snap-mandatory overflow-x-hidden">
        @foreach ($posts as $post)
            @php
                $cover = $post->media()->first();
            @endphp
            <li
                class="h-[calc(100vh_-_5rem)] max-w-sm mx-auto w-full cursor-pointer rounded-lg relative group shrink-0 snap-center snap-always grid grid-cols-12 gap-4">
                {{-- main --}}
                        ...
                        @else
                            <button wire:click='togglePostLike({{ $post->id }})'>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.9" stroke="currentColor" class="w-8 h-8">
