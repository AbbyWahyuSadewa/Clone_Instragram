                ...
                @switch($notification->type)
                            ...
                            <div class="col-span-3">
                                @if (auth()->user()->isFollowing($user))
                                    <button wire:click='toggleFollow({{ $user->id }})'
                                        class="font-bold text-sm bg-gray-100 text-black/90 px-3 py-1 rounded-lg">Following</button>
                                @else
                                    <button wire:click='toggleFollow({{ $user->id }})'
                                        class="font-bold text-sm bg-blue-500 text-white px-3 py-1 rounded-lg">Follow</button>
                                @endif
                            </div>
                        </div>
                    @break
                        ...
                        <div class="grid grid-cols-12 gap-2 w-full">
                            ...
                            <div class="col-span-7 font-medium">
                                ...
                                <a href="{{ route('post', $post->id) }}">
                                    Liked your post
                                    <span
                                        class="test-gray-400">{{ $notification->created_at->shortAbsoluteDiffForHumans() }}</span>
                                </a>
                            </div>
                    ...
                    @case('App\Notifications\NewCommentNotification')
                        ...
                        <div class="grid grid-cols-12 gap-2 w-full">
                            ...
                            <div class="col-span-7 font-medium">
                                <a
                                    href="{{ route('profile.home', $user->username) }}"><strong>{{ $user->name }}</strong></a>
