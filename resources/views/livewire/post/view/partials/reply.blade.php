<div wire:key="reply-{{ $reply->id }}" class="flex items-center w-11/12 ml-auto gap-3 py-2">
    <x-avatar wire:ig src="https://source.unsplash.com/500x500?/face-{{ rand(1, 10) }}" class="h-8 w-8 mb-auto" />
        ...
        <div class="col-span-7 flex gap-2 text-sm items-center text-gray-700">
            ...
            <span class="font-bold">
                @if ($reply->totalLikers > 0 && !$reply->hide_like_view)
                    {{ $reply->totalLikers }}
                    {{ $reply->totalLikers > 1 ? 'likes' : 'like' }}
                @endif
            </span>
